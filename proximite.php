...
<main>
    ...
    <section class="geo-info">
        <?php if ($geo_ok) : ?>
            <p>
                Position trouvée :
                <strong><?= htmlspecialchars($geo_ville) ?></strong>
            ...
            <p class="notice">
                <em>La localisation par IP est approximative.</em>
            </p>
        <?php endif; ?>
    </section>
    ...
    <section class="filtre">
        <form method="get" action="proximite.php">
            ...
            <select name="carburant" id="carburant">
                ...
                <?php foreach (['SP95', 'SP98', 'Gazole', 'E10', 'GPL', 'E85'] as $c) : ?>
                    <option value="<?= $c ?>" <?= ($carburant === $c) ? 'selected="selected"' : '' ?>>
                        <?= $c ?>
                    </option>
                <?php endforeach; ?>
            </select>
    ...
    <section class="resultats">
        <h3>
            <?= min(count($stations), $max_stations) ?> / <?= count($stations) ?>
            station<?= count($stations) > 1 ? 's' : '' ?>
        ...
        <?php if (count($stations) > $max_stations) : ?>
            <p class="notice">
                Seules les <?= $max_stations ?> stations les moins chères sont affichées.
                Pour chercher par ville précise, utilisez la
                <a href="index.php">recherche classique</a>.
            </p>
        <?php endif; ?>
    </section>
